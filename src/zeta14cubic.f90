...
program compute_hardy_fast
  ...
  integer       :: ip,K,icj(0:50),MIT,totblock,irsp(7,2),i,mc,m1(50),pblocks,mmax,istartblock
  integer(dp1)  :: NC,jsum,RSN,L(0:50),KSET
...
!     a data file called "inputs2.nml". Obviously the source code must have access to this file. 


   open(2,file='inputs2.nml',status='old')
   do i=1,9
    read(2,4001) line
   enddo
   read(2,4002) et
...
!  suit your needs. Varying parameters K and totblock produces minor changes in run times but typically nothing major.
  ...
  IF (KSET.EQ.0) THEN
     ...
     totblock=MAX0(25,FLOOR(((log(et)/log(0.005))**0.25)*(3.545*log(t))-61.8,dp1))
  ELSE
     K=400
     totblock=188
  ENDIF   
...
!   subsequently re-discovered by Siegel, 73 years later, amongst Riemann's unpublished papers stored in the Gottingen University Library.
  ...
  write(2,3011) 'Riemann-Sieg NT frac= ',rsfrac
  tsum=tsum+rstime
  zsum=zsum+rszsum
...
3003 FORMAT(A24,E28.21,2X,A37,I3)
...
subroutine alphasum(MTM,rae,pcsum)
  ...
  integer       :: K,ip,mc,i
  integer(dp1)  :: MTM,jj
  real(dp)      :: rae,pcsum,p,sp,t,a,et,e,s,amp,s1,pc,xx,wm,wp,initialcoeff(3)
  real(dp)      :: rs,ht,ain,Y,pc2,con2,tpp,tpm,initialcoeffcc(3)
  real(dp)      :: cc,cc1,tse
...
subroutine mgausssum(m,ip,csum)
  ...
  integer       :: m,ip,MIT,icj(0:50),k,j,m1(50),mmax,jj
  integer(dp1)  :: L(0:50),i
...
subroutine q(m,nit,ip,qq)
  ...
  integer       :: m,nit,ip,j,ip1,icj(0:50),MIT,m1(50),jbot,mmax
  integer(dp1)  :: L(0:50),i
  real(dp)      :: p,sp,tpp,tpm,e3,sum,ps1,ps2
  real(dp)      :: phicoeff(0:15,50),xr(50),fracL(0:50),sx,con1,con2,con3,z,wm,c,beta
...
! This routine calculates the RSremainder(t) term. The result is stored in
! rzsum.       
...
subroutine t_grabber(t)
  ...
  ! Y = '1.0000000000015907542300844918921000000000e24'  ! keep decimal digits 40 long
...
subroutine real2pari(r,r_pari)
  ...
  !       CALL pari_init(10000000_8, 2_8)
    ...
    READ(Y(3:16)   , '(I14)'  )  c0
    READ(Y(17:26)  , '(I10)'  )  c1            ! first digit and 10-digit pieces of r
    READ(Y(28:37)  , '(I10)'  )  c2
    READ(Y(38:47)  , '(I10)'  )  c3
    READ(Y(48:57)  , '(I10)'  )  c4
    READ(Y(58:67)  , '(I10)'  )  c5
...
subroutine pari_tse(rae,tse)
  ...
  ! where pc=2*(s^2)*(sqrt(1-1/(s^2))+1)-1 and s=rae/a>1. It requires high
